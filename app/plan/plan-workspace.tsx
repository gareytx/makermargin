...
function SuccessResult({ result }: { result: PortfolioSuccessResult }) {
  ...
  return <section aria-labelledby="results-heading" className="space-y-7"><div><h2 id="results-heading" className="text-3xl font-bold">Production plan results</h2><p className="mt-1 text-slate-300">{periodNames[result.period.type]} · {result.period.label}</p></div>
    ...
    {result.warnings.length ? <WarningMessages warnings={result.warnings} products={result.products} /> : null}<Messages title="Engine explanations" messages={result.explanations} />
...
function Limiting({ capacity }: { capacity: PortfolioSuccessResult["capacity"] }) {
  ...
  const primaryKeys = new Set(capacity.primaryLimitingResources.map(resourceKey));
  const additionalNearTies = capacity.nearTiedResources.filter((resource) => !primaryKeys.has(resourceKey(resource)));
  return <div className="mt-4 rounded border border-slate-700 p-4"><h4 className="font-semibold">Limiting resources</h4>{capacity.primaryLimitingResources.length ? <p className="mt-2">Primary: {capacity.primaryLimitingResources.map(label).join(", ")}</p> : <p className="mt-2 text-slate-300">No limiting resource can be identified from the supplied capacities.</p>}{additionalNearTies.length ? <p className="mt-1 text-sm text-slate-300">Additional resources within the engine&apos;s near-tie range: {additionalNearTies.map(label).join(", ")}</p> : null}</div>;
}

function resourceKey(resource: PortfolioResourceReference) { return `${resource.resourceType}:${resource.key}`; }
...
function WarningMessages({ warnings, products }: { warnings: PortfolioWarning[]; products: Array<{ productId: string; productName: string }> }) { const names = new Map(products.map(({ productId, productName }) => [productId, productName])); return <section><h3 className="text-2xl font-semibold">Warnings</h3><ul className="mt-3 space-y-3 rounded border border-amber-800 bg-amber-950/20 p-4">{warnings.map((warning, index) => <li key={`${warning.code}-${warning.productId}-${index}`} className="rounded border border-amber-900 p-3"><p><strong>{names.get(warning.productId) ?? "Saved product"}:</strong> {warning.message}</p><Link href={`/products/${warning.productId}#production-cash-profile`} className="mt-2 inline-block font-semibold text-emerald-400">Review Production &amp; Cash Profile</Link></li>)}</ul></section>; }
