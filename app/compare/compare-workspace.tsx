...
import { comparisonMetricLabels, formatComparisonValue, formatMetric } from "@/lib/comparison-formatting";
import {
...
const leaderDefinitions: Array<[LeaderCategory, string, string, string]> = [
  ...
  ["highestOwnerBenefitPerLaborHour", "Highest owner benefit per hands-on owner labor hour", "Greatest owner labor compensation plus business profit per hands-on owner labor hour.", "Production details are needed to compare owner benefit per hands-on owner labor hour."],
  ...
  ["fastestActiveProduction", "Lowest hands-on owner labor", "Least total hands-on owner labor per sellable product.", "Production details are needed to compare hands-on owner labor."],
];
...
const metricGroups: Array<[string, MetricRow[]]> = [
  ["Production and throughput", [
    ["activeLaborMinutesPerSellableProduct", "Total hands-on owner labor per sellable product"], ["activeLaborMinutesPerBatch", "Total hands-on owner labor per representative batch"],
    ...
    ["unitsPerLaborHour", "Units per hands-on owner labor hour"], ["unitsPerMachineHour", "Units per occupied machine hour"],
  ]],
  ["Efficiency", [
    ["businessProfitPerLaborHour", "Business profit per hands-on owner labor hour"], ["ownerEconomicBenefitPerLaborHour", "Owner economic benefit per hands-on owner labor hour"],
...
const batchLabels: Record<BatchSubleaderKey, string> = {
  ...
  highestOwnerBenefitPerLaborHour: "Highest owner benefit per hands-on owner labor hour",
...
function CapacityResults({ result, name }: { result: ProductComparisonOutput; name: (id: string) => string }) {
  return <section aria-labelledby="capacity-results-heading"><h2 id="capacity-results-heading" className="text-2xl font-semibold">Capacity and bottlenecks</h2><p className="mt-1 text-sm text-slate-400">Maximums are planning estimates from the supplied period limits, not production commitments. Each percentage is the share of period capacity required for one representative batch.</p><div className="mt-4 grid gap-4 lg:grid-cols-2">{Object.entries(result.bottlenecksByProduct).map(([productId, bottleneck]) => {
    ...
    return <article key={productId} className="rounded border border-slate-700 bg-slate-900 p-4"><h3 className="break-words text-lg font-semibold">{name(productId)}</h3>{available.length ? <dl className="mt-3 space-y-3">{available.map(([resource, metric]) => { const capacity = bottleneck.capacities[resource]; return <div key={resource}><dt className="text-sm text-slate-400">{resourceLabels[resource]}</dt><dd>{metric.status === "available" ? <><p><span className="font-semibold">{formatMetric(metric)}</span><span className="ml-2 text-xs text-slate-400">for one batch</span></p>{capacity.status === "available" ? capacity.nonLimiting ? <p className="text-sm text-slate-300">Non-limiting - the batch requires none of this resource.</p> : <p className="text-sm text-slate-300">Maximum: <strong>{capacity.maxCompleteBatches} complete batches</strong> / <strong>{capacity.maxSellableProducts} sellable products</strong></p> : null}</> : null}</dd></div>; })}</dl> : <div className="mt-3"><p className="font-semibold text-amber-300">Capacity analysis unavailable</p><p className="mt-1 text-sm text-slate-400">Representative batch, hands-on owner labor, and applicable upfront-cash details are needed before this product can be evaluated against supplied limits.</p></div>}{unavailable.length ? <details className="mt-3"><summary className="cursor-pointer text-sm font-semibold text-emerald-400 focus-visible:outline-2 focus-visible:outline-emerald-400">{available.length ? "Other capacity data unavailable" : "Review unavailable capacity details"}</summary><ul className="mt-2 space-y-2 text-sm">{unavailable.map(([resource, metric]) => <li key={resource}><p className="font-medium">{resourceLabels[resource]}</p><p className="text-slate-400">{metric.status === "unavailable" ? capacityReason(resource, metric.reason.message) : ""}</p></li>)}</ul></details> : null}{bottleneck.status === "available" ? <div className="mt-4 border-t border-slate-700 pt-3 text-sm"><p><strong>Limiting resource:</strong> {bottleneck.primaryResources.map((resource) => resourceLabels[resource]).join(" and ")}</p><p className="mt-1"><strong>Near-tied resources:</strong> {bottleneck.nearTiedResources.length ? bottleneck.nearTiedResources.map((resource) => resourceLabels[resource]).join(", ") : "None"}</p></div> : <p className="mt-4 text-sm text-amber-300">A limiting resource cannot be determined from the available positive resource demands.</p>}</article>;
  })}</div></section>;
}
