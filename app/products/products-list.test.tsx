...
describe("saved products list", () => {
  ...
  it("shows saved metrics and accessible actions", () => { render(<ProductsList initialProducts={[product]} />); expect(screen.getByText("Journal")).toBeTruthy(); expect(screen.getByText("Recommended price")).toBeTruthy(); expect(screen.getByRole("link", { name: "Open" })).toBeTruthy(); expect(screen.getByRole("link", { name: "Compare products" }).getAttribute("href")).toBe("/compare"); });
